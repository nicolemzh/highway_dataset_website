...
function DatasetTable({ datasets }) {
  ...
  useEffect(() => {
    datasets.forEach((dataset) => {
      if (dataset.wave) {
        fetch(`http://localhost:5001/api/waves/${dataset.id}`)
      ...
      if (dataset.preprocessed) {
        fetch(`http://localhost:5001/api/preprocessed/${dataset.id}`)
          .then((response) => response.json())
          .then((data) => {
          ...
          {datasets.map((dataset) => (
            <tr key={dataset.id}>
              ...
              <td style={{ position: 'relative', width: '400px' }}>
                <a href={dataset.preprocessed} download>
